...
void starting_position(GameInfo_t* game_data) {  // стартовые позиции

  game_data->pause = 0;

  game_data->field = calloc(20, sizeof(int*));
  for (int i = 0; i < 20; ++i) {
    game_data->field[i] = calloc(10, sizeof(int));
  }

  game_data->next = calloc(2, sizeof(int*));
  for (int i = 0; i < 2; ++i) {
    game_data->next[i] = calloc(5, sizeof(int));
  }
...
int spawn(GameInfo_t* data_tetris) {  //циферки запись для некс
...
void write_1(GameInfo_t* g, int y,
...
void write_0(GameInfo_t* g, int y,
